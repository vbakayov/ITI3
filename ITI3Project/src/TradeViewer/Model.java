...
public class Model {
	...
	// contains only the rows from the original dataset that pass the currently
	// set filters -
	// filtering is achieved by making all views use this dataset instead of the
	// default one
	...
	/*
	 * Constructor creates the main data structures and then loads data in the
	 * given file. After this, children should be null but the other ArrayLists
	 * should be filled with the initial data for analysis.
	 */
	...
	/*
	 * addChild Adds a new view-controller to the set of children of this model
	 */
	...
	protected void load_xlms(String filename2, Boolean addLabels) {
		...
		try {
			...
			file.close();
			//MessageInfo.processMessage(JOptionPane.INFORMATION_MESSAGE, "The file has been loaded successfully");
		} catch (InvalidFormatException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (FileNotFoundException e) {
			e.printStackTrace();
				MessageInfo.processMessage(JOptionPane.ERROR_MESSAGE, "The system cannot find the file specified");
			}		
		catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public void printRow(ArrayList row) {
		...
		while (rowIter.hasNext()) {
			System.out.println(labelIter.next().toString() + " = " + rowIter.next().toString());
			System.out.println("***************************");
		}
	}
	...
	public void CopyRowXLSXFile(String absolutePath) {
		...
		String fileNameIn =filename.substring(filename.lastIndexOf('\\')+1, filename.length());
		String fileNameOut =absolutePath.substring(absolutePath.lastIndexOf('\\')+1, absolutePath.length());

		MessageInfo.processMessage(JOptionPane.INFORMATION_MESSAGE, "The filtered "+ sizeAvailable+" row(s) have been saved in " + fileNameOut +
																		" and deleted from "+ fileNameIn);
		}
